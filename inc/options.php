...
function toast_elegant_sanitize_options($input)
{
    ...
    if ($sanitized['duration'] < 1000)
        $sanitized['duration'] = 1000;
    if ($sanitized['duration'] > 10000)
        $sanitized['duration'] = 10000;
...
function toast_elegant_options_page()
{
                ...
                <div id="appearance" class="tab-content">
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="range" name="toast_elegant_options[border_radius]" min="0" max="30"
                                    value="<?php echo esc_attr($options['border_radius']); ?>" class="toast-elegant-range">
                ...
                <div id="position" class="tab-content">
                    ...
                    <table class="form-table">
                        <tr>
                            ...
                            <td>
                                <fieldset>
                                    ...
                                    <label><input type="radio" name="toast_elegant_options[position]" value="top-center"
                                            <?php checked($options['position'], 'top-center'); ?>> ⬆️ 顶部居中</label><br>
                                    <label><input type="radio" name="toast_elegant_options[position]" value="top-right"
                                            <?php checked($options['position'], 'top-right'); ?>> ↗️ 右上角</label><br>
                                    <label><input type="radio" name="toast_elegant_options[position]" value="bottom-left"
                                            <?php checked($options['position'], 'bottom-left'); ?>> ↙️ 左下角</label><br>
                                    <label><input type="radio" name="toast_elegant_options[position]" value="bottom-center"
                                            <?php checked($options['position'], 'bottom-center'); ?>> ⬇️ 底部居中</label><br>
                                    <label><input type="radio" name="toast_elegant_options[position]" value="bottom-right"
                                            <?php checked($options['position'], 'bottom-right'); ?>> ↘️ 右下角</label>
                                </fieldset>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <input type="number" name="toast_elegant_options[position_top_desktop]"
                                    value="<?php echo esc_attr($options['position_top_desktop']); ?>" min="0" max="500"
                                    class="small-text"> px
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="number" name="toast_elegant_options[position_top_mobile]"
                                    value="<?php echo esc_attr($options['position_top_mobile']); ?>" min="0" max="500"
                                    class="small-text"> px
                        ...
                        <tr>
                            ...
                            <td>
                                <fieldset>
                                    <label><input type="radio" name="toast_elegant_options[animation]" value="slide-fade"
                                            <?php checked($options['animation'], 'slide-fade'); ?>> 滑动淡入</label><br>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="range" name="toast_elegant_options[duration]" min="1000" max="10000" step="500"
                                    value="<?php echo esc_attr($options['duration']); ?>" class="toast-elegant-range">
    ...
    <script>
        ...
        function testToast(type) {
            ...
            // 根据类型设置不同的演示文案
            switch (type) {
                case 'success': message = '✨ 设置已保存成功！'; break;
                case 'error': message = '❌ 连接服务器失败，请重试。'; break;
                case 'warning': message = '⚠️ 请注意，此操作不可逆！'; break;
                case 'info': message = 'ℹ️ 您有一条新的未读消息。'; break;
                case 'loading': message = '⏳ 系统正在处理中...'; break;
            }

            // 特殊处理 loading 类型：3秒后自动关闭并提示成功
            if (type === 'loading') {
                var toast = ToastElegant.loading(message);
                setTimeout(function () {
                    toast.close(); // 关闭 loading
                    ToastElegant.success('处理完成！'); // 显示成功
                }, 3000);
                return;
            }

            // 其他类型正常显示
            ToastElegant[type](message);
        }
    </script>
    <?php
}